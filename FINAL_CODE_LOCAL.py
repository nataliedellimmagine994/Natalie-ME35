...
def calc_motion(size, target, pos):
    ...
    print(f"size: {size} speed: {speed} pos:{pos} kp:{kp_Pos_term} ki:{ki_Pos_term}")
    # Limits
...
async def main():
    ...
    while True:
        ...
        if state == "finding_ball":
            ...
            # First drive forward into middle of field (only once)
            #if not pipeline_running:
                #await send_motor_ble(99, 99)
            ...
            if tag is not None: 
                ...
                print("no tag in view, rotating right")
                await turn_45("right")
                continue
